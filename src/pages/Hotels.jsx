...
const allHotels = [
  {
    ...
    amenities: ["Cancellation Policy", "Breakfast Included", "Free Wifi", "Swimming Pool"],
    featured: true
  },
  {
    ...
    amenities: ["Cancellation Policy", "Breakfast Included", "Spa", "Restaurant"],
    featured: true
  },
  {
...
function Hotels() {
  ...
  const handleSort = (criteria) => {
    ...
    switch(criteria) {
      case 'price-low':
        sortedHotels.sort((a, b) => parseInt(a.price.replace(/[^0-9]/g, '')) - parseInt(b.price.replace(/[^0-9]/g, '')));
        break;
      case 'price-high':
        sortedHotels.sort((a, b) => parseInt(b.price.replace(/[^0-9]/g, '')) - parseInt(a.price.replace(/[^0-9]/g, '')));
        break;
      case 'rating':
    ...
    >
      ...
      <div className="relative pt-32 pb-16">
        <div className="relative flex flex-col items-center justify-center text-center px-4">
          ...
          Here we are empowering the wildlife travellers with instant hotel and resort booking with comprehensive choices in Kanha National Park. We have tie-up with vast network of hotels in Kanha which include both luxury and budget hotels.
          </p>
        </div>
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="relative">
          <div className="absolute inset-0 bg-emerald-900/30 backdrop-blur-lg rounded-3xl -m-8"></div>
          
          

          {/* Filter and Sort Section */}
          <div className="relative mb-8 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-4 bg-emerald-900/50 p-4 rounded-xl">
              {/* Star Filter */}
                ...
                <div className="flex gap-3">
                  {[3, 4, 5].map(star => (
                    <button
                      ...
                      className={`flex items-center gap-1 px-4 py-2 rounded-lg transition-colors duration-200
                        ${selectedStars.includes(star) 
                          ? 'bg-emerald-500 text-white' 
                          : 'bg-emerald-900/50 text-emerald-100 hover:bg-emerald-800/50'}`}
                    >
                      <Star className={`h-4 w-4 ${selectedStars.includes(star) ? 'fill-white' : 'fill-emerald-400'}`} />
                      <span>{star} Star</span>
                    </button>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <option value="" className="bg-emerald-800 text-emerald-100">Sort By</option>
                  <option value="price-low" className="bg-emerald-800 text-emerald-100">Price: Low to High</option>
                  <option value="price-high" className="bg-emerald-800 text-emerald-100">Price: High to Low</option>
                  <option value="rating" className="bg-emerald-800 text-emerald-100">Rating</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="relative space-y-6">
            {hotels.map((hotel, index) => (
              <div key={index} className="bg-emerald-900/30 backdrop-blur-lg rounded-2xl overflow-hidden border border-emerald-500/20">
                <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <div className="p-6 md:col-span-2">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="text-right">
                        <div className="flex items-center gap-1 mb-2">
                          ...
                          <span className="text-emerald-300">({hotel.reviews} reviews)</span>
                        </div>
                    ...
                    <p className="text-emerald-100 text-sm mb-4">{hotel.address}</p>
